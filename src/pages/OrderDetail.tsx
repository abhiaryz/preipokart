...
import { CompanyLogo, Field, InlineNotice, QueryStatus } from '../components/ui';
import { api, errorMessage } from '../api';
import { useApi } from '../hooks/useApi';
import { STATUS_LABEL, STATUS_STEPS, canCancel, canEdit } from '../lib/format';
import posthog, { isPostHogConfigured } from '../posthog';

export default function OrderDetail() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { data: order, error, loading, reload } = useApi(() => api.getOrder(id!), [id], Boolean(id));
  ...
  const stepIndex = order ? STATUS_STEPS.indexOf(order.status === 'cancelled' ? 'open' : order.status) : -1;
  const editable = order ? canEdit(order.status, order.canEdit) : false;
  const cancellable = order ? canCancel(order.status, order.canCancel) : false;
  ...
  const onSave = async (e: FormEvent) => {
    ...
    if (nextPrice <= 0 || nextQty <= 0) {
      setFormError('Enter a valid price and number of shares.');
      setNotice('');
      return;
    }
    setBusy(true);
    try {
      await api.updateOrder(order.id, { price: nextPrice, quantity: nextQty });
      ...
      setFormError('');
      setNotice('Order updated. It stays in the book at the new price.');
      await reload();
    } catch (err) {
      setNotice('');
      setFormError(errorMessage(err, 'This order can no longer be changed.'));
    } finally {
      setBusy(false);
    }
  };
  ...
  const onCancel = async () => {
    ...
    try {
      ...
      setFormError('');
      setNotice('Order cancelled. It will not match with anyone.');
      await reload();
    } catch (err) {
      setNotice('');
      setFormError(errorMessage(err, 'This order can no longer be cancelled.'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <QueryStatus loading={loading || !order} error={null}>
      {order ? (
        <div className="mx-auto max-w-3xl">
          ...
          {editable ? (
            <form className="card mb-4 space-y-4 p-5" onSubmit={(e) => void onSave(e)}>
          ...
          {cancellable ? (
            <div className="card p-5">
              ...
              <button type="button" className="btn-secondary mt-4" onClick={() => void onCancel()} disabled={busy}>
                Cancel this order
              </button>
            </div>
