...
export default function Blog() {
  const { data, error, loading } = useApi(() => api.listBlog(), []);
    ...
    <div className="mx-auto max-w-6xl">
      <header className="mb-8 max-w-[65ch]">
        ...
        <p className="mt-2 font-body-md text-body-md text-on-surface-variant">
          Listed-market cues, policy, and how they relate to unlisted shares.
        </p>
      </header>
        ...
        {featured ? (
          ...
          >
            {featured.cover ? (
              ...
              <div className="h-56 bg-surface-container-low md:min-h-[280px]" />
            )}
          ...
          <p className="card p-6 text-sm text-on-surface-variant">No posts yet.</p>
        )}
        ...
        <ul className="grid gap-6 sm:grid-cols-2">
          {rest.map((post) => (
            <li key={post.slug}>
              ...
              >
                {post.cover ? <img src={post.cover} alt="" className="h-40 w-full object-cover" /> : null}
