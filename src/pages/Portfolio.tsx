...
import { STATUS_LABEL, getOrders, subscribeOrders } from '../data/orders';
import { buildPortfolio, formatInr, formatSignedInr, type HoldingRow } from '../data/portfolio';
...
export default function Portfolio() {
  ...
  const snapshot = useMemo(() => buildPortfolio(orders), [orders]);
  const [sortKey, setSortKey] = useState<SortKey>('marketValue');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc');

  const sorted = useMemo(() => {
    const next = [...snapshot.holdings];
    next.sort((a, b) => {
      const av = a[sortKey];
      const bv = b[sortKey];
    ...
    return next;
  }, [snapshot.holdings, sortKey, sortDir]);
  ...
  const empty = snapshot.holdings.length === 0;
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Portfolio totals">
        ...
        <article className="card p-4">
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            Escrow in flight {formatInr(snapshot.escrowIn)}
          </p>
        </article>
      </section>
